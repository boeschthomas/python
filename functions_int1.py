...
def randint(min=0, max=100):
  possible_range = max - min              #First, we start by getting the possible range of numbers.
  return round(random.random() * possible_range + min)  #Multiplying this number will give us a number that is no greater than the range, but no less than 0.
